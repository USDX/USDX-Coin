...
uint256 hashGenesisBlock("0x0000030e4928d7c1ad71d6841a803e4441edb4d206b0ea52e3eb80ff44b09e98");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // USDX: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "USDX Signed Message:\n";
...
{
    // USDX: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if (nHeight == 1){
        nSubsidy = 3000000 * COIN; // pre-mine
    }
    else if(nHeight > 1){
        nSubsidy = 5 * COIN;
    }
...
static const int64 nTargetTimespan = 1 * 60; //every block
static const int64 nTargetSpacing = 1 * 60; //every minute
...
{
    // USDX: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    //vMerkleTree: b0019d92bc054f7418960c91e252e7d24c77719c7a30128c5f6a827c73095d2a
                ...
                const char* pszTimestamp = "Pentagon balked at U.S. border troops building detention facilities 11-5-2018";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 5 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04b508029ab3be05a1d341627c683cebba10b7213f2a5df88c5e30a3dec0efa598dedafd3db2bf47d0616257d47554975aebedf95c8956a71d2b0e6a84d03337f8") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1541463223;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1673702;

        if (fTestNet)
        {
            block.nTime    = 1541463201;
        ...
        assert(block.hashMerkleRoot == uint256("0x72cb5f0d59dd42a377589e2afeb14dc6e9345f795fca8c3284216a9cc1bbfadf"));
...
unsigned char pchMessageStart[4] = { 0xf5, 0xc1, 0xb3, 0xdd }; // USDX: increase each by adding 6 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("USDXMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("USDXMiner : ProcessBlock, block not accepted");
    }
